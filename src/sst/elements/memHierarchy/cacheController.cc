...
void Cache::sendNACK(MemEvent* event) {
    if (event->isHighNetEvent())        coherenceMgr_->sendNACK(event, true, getCurrentSimTimeNano());
    else if (event->isLowNetEvent())    coherenceMgr_->sendNACK(event, false, getCurrentSimTimeNano());
    else
...
void Cache::processIncomingNACK(MemEvent* origReqEvent) {
    ...
    if (origReqEvent->fromHighNetNACK())       coherenceMgr_->resendEvent(origReqEvent, true);
    else if (origReqEvent->fromLowNetNACK())   coherenceMgr_->resendEvent(origReqEvent, false);
    else
