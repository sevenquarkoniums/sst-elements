...
#include "DflyHybridRnAllocator.h"
...
DflyHybridRnAllocator::DflyHybridRnAllocator(const DragonflyMachine & mach)
  : DragonflyAllocator(mach)
{
    rng = new SST::RNG::MersenneRNG();
}

DflyHybridRnAllocator::~DflyHybridRnAllocator()
{
    delete rng;
}

std::string DflyHybridRnAllocator::getSetupInfo(bool comment) const
{
    std::string com;
    if (comment) {
        com = "# ";
    ...
    return com + "Dragonfly HybridRn Allocator";
}

#include <iostream>
using namespace std;

AllocInfo* DflyHybridRnAllocator::allocate(Job* j)
{
    if (canAllocate(*j)) {
        ...
        //it will simply spread randomly across the machine.
        int groupID = rng->generateNextUInt32() % dMach.numGroups;
        for (int i = 0; i < jobSize; i++) {
            int localNodeID = rng->generateNextUInt32() % nodesPerGroup;
            while (true) {
                int nodeID = groupID * nodesPerGroup + localNodeID;
                if ( dMach.isFree(nodeID) && occupiedNodes.find(nodeID) == occupiedNodes.end() ) {
                    ai->nodeIndices[i] = nodeID;
